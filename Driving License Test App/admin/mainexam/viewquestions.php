...
/*if ($_SESSION['isloggedin'] != 1) {
    ...
    <?php
    $answersquery = "SELECT * FROM `mainexamchoices` WHERE is_correct = 1";
    $answersqueryrun = $mysqli->query($answersquery);
    $questionsquery = "SELECT * FROM `mainexamquestions`";
    ...
    if (!$questionsqueryrun) {
        ...
        for ($i = 0; $i < $nbrows; $i++) {
            ...
            $textOrImage = $answers['text_'] = "" ? $answers['image_'] : $answers['text_'];
            echo "<td>$questions[text_]</td>";
            echo "<td>$textOrImage</td>";
